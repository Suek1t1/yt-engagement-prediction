"""
タイトル・タグの単語をone-hot encodingするスクリプト（上位500語）
...
# ── 5. 上位500語に絞る ───────────────────────────────────────────
all_words = [w for tokens in df["all_tokens"] for w in tokens]
top_words = [word for word, _ in Counter(all_words).most_common(500)]

df["all_tokens_filtered"] = df["all_tokens"].apply(
    lambda tokens: list(set(t for t in tokens if t in top_words))
)

# ── 6. MultiLabelBinarizer で one-hot encoding ────────────────────
mlb = MultiLabelBinarizer(classes=top_words)
...
# ── 7. CSV出力 ───────────────────────────────────────────────────
result.to_csv("onehot_features_top500.csv", index=False)
print("CSV保存完了: onehot_features_top500.csv")
